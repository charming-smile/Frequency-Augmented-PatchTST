"""
PatchTST_FreqEmbed with non-negative gating: alpha = softplus(param).

Same as PatchTST_FreqEmbed, except the learned fusion weight cannot go
negative, so frequency information can only be added, never subtracted.
"""
...
class FrequencyAugmentedPatchEmbedding(nn.Module):
    """Same as PatchTST_FreqEmbed, but alpha is forced non-negative."""
    ...
    def forward(self, x):
        ...
        freq_mag = torch.fft.rfft(x, dim=-1).abs()
...
class Model(nn.Module):
    """PatchTST with non-negative frequency gating (ablation)."""
    ...
    def __init__(self, configs, patch_len=16, stride=8):
        ...
        if self.task_name in ("long_term_forecast", "short_term_forecast"):
            ...
            raise NotImplementedError(
                "PatchTST_FreqEmbedPos supports long/short term forecast only"
            )
